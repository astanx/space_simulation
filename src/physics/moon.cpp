...
: OrbitalObject(centralBody, mu, radius, keplerElements, false), hapkeParameters(hapkeParameters), ModelSource(static_cast<const PositionSource &>(*this), radius * VISUAL_RADIUS_SCALE)
{
  if (this->useTrail)
    this->generateTrail();
}
