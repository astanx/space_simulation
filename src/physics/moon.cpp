...
Moon::Moon(OrbitalObject *centralBody, double mu, Radii radii, const KeplerElements &keplerElements, const HapkeParameters &hapkeParameters, TidalParameters tidalParameters, GravityField gravityField)
    : OrbitalObject(centralBody, mu, radii, keplerElements, tidalParameters, gravityField, false), hapkeParameters(hapkeParameters), ModelSource(static_cast<const TransformSource &>(*this), radii.mean)
{
  if (this->useTrail)
    this->generateTrail();
}
